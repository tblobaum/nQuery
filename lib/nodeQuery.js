...
module.exports = function (client, conn) {
    ...
    this.nodeQuery = function (context, isReady) {
        conn.nQhref = context;
        conn.isReady = isReady;
        var nQurl = url.parse(context);
        conn.nQpath = nQurl.pathname;
        conn.nQstream = '$' + conn.id + ' ' + conn.nQpath;
        if (module.exports.debug) console.log(conn.nQstream, 'routing to ', conn.nQpath);
        conn.emit(conn.nQpath, isReady);
        conn.emit('$', isReady);
    };
    
    conn.on('end', function () {
        if (module.exports.debug) console.log('Ending conn.nQstream:' + conn.nQstream);
        if (module.exports.debug) console.log('conn.nQpath:' + conn.nQpath);
    });
